(function () {
            ...
            try {
                if (data.callback) window.mqq.ui.setOnShareHandler(function (type) {
                    if (type == 3 && (data.swapTitle || data.WXconfig &&
                        data.WXconfig.swapTitleInWX)) info.title = data.summary; else info.title = data.title;
                    window.mqq.ui.shareMessage($.extend(info, {share_type: type, back: true}), function (result) {
    ...
    if (typeof define === "function" && (define.cmd || define.amd)) {
        if (define.cmd) require = seajs.use; else if (define.amd) require = window.require;
        define(function () {
            return init
        })
    } else {
